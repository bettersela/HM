...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) : instr list =
    ...
    | If(e, stmt1, stmt2) ->
      ...
      @ [Label labend]
                                                               (*add*)
    | Switch(e,cases) ->
      printfn "cStmt = %A, stmt = %A, cStmt.type = %A, stmt.type = %A "  cStmt stmt (cStmt.GetType()) (stmt.GetType());
...
and cExpr (e : expr) (varEnv : varEnv) (funEnv : funEnv) : instr list =
    ...
    | Assad(acc, e)  -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ cExpr e varEnv funEnv @ [ADD] @[STI]
    | Asssu(acc, e)  -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ cExpr e varEnv funEnv @ [SUB] @[STI]
    | Assmu(acc, e)  -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ cExpr e varEnv funEnv @ [MUL] @[STI]
    | Assdi(acc, e)  -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ cExpr e varEnv funEnv @ [DIV] @[STI]
    | Assmo(acc, e)  -> cAccess acc varEnv funEnv @ [DUP] @ [LDI] @ cExpr e varEnv funEnv @ [MOD] @[STI]
